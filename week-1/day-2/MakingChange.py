...
def change_possibilities(amount, denominations):
	lngth= len(denominations)
	ans_values = [[0 for i in range(lngth)] for j in range(amount+1)]
	if lngth==0:
		return 0;
	...
	for i in range(1,amount+1):
		for j in range(lngth):
			r = ans_values[i - denominations[j]][j] if i-denominations[j] >= 0 else 0
			a = ans_values[i][j-1] if j >= 1 else 0
			ans_values[i][j]=r+a
	return ans_values[amount][lngth-1]
