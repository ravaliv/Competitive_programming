import unittest
def max_duffel_bag_value(cake_vals, duffelcapacity):
    max_capacity = [0]*(duffelcapacity+1)
    for cc in range(duffelcapacity + 1):
        c_max=0
        for cake_weight, cake_value in cake_vals:
            if cake_weight ==0 and cake_value!=0:
                return float('inf')
            if cake_weight <= cc:
                max_value_cake = (cake_value+max_capacity[cc-cake_weight])
                c_max = max(max_value_cake, c_max)
        max_capacity[cc] = c_max
    return max_capacity[duffelcapacity]
